// Based on https://github.com/Zondax/ledger-rs/tree/4ed4cfdef0ae8b40e8997d849a3262bcf00c7d3c/ledger-transport-hid

use std::{fmt, io::Cursor, ops::Deref, sync::Mutex};
...
impl fmt::Display for LedgerHIDError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            LedgerHIDError::DeviceNotFound => write!(f, "Ledger device not found"),
            LedgerHIDError::Comm(s) => write!(f, "Communication error: {}", s),
            LedgerHIDError::Io(e) => write!(f, "I/O error: {}", e),
            LedgerHIDError::Hid(e) => write!(f, "HID error: {}", e),
            LedgerHIDError::UTF8(e) => write!(f, "UTF-8 error: {}", e),
        }
    }
}

impl std::error::Error for LedgerHIDError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self {
            LedgerHIDError::Io(e) => Some(e),
            LedgerHIDError::Hid(e) => Some(e),
            LedgerHIDError::UTF8(e) => Some(e),
            _ => None,
        }
    }
}
...
impl From<hidapi::HidError> for LedgerHIDError {
    fn from(e: hidapi::HidError) -> Self {
        LedgerHIDError::Hid(e)
    }
}

impl From<std::str::Utf8Error> for LedgerHIDError {
    fn from(e: std::str::Utf8Error) -> Self {
        LedgerHIDError::UTF8(e)
    }
}
...
impl TransportNativeHID {
    ...
    pub fn exchange(&self, command: &APDUCommand) -> Result<APDUAnswer<Vec<u8>>, LedgerHIDError> {
        let device = self.device.lock().expect("HID device poisoned");

        Self::write_apdu(&device, LEDGER_CHANNEL, &command.encode())?;

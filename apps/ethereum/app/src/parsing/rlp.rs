...
//! - No allocations for small items
...
pub enum RlpItem<'a> {
    ...
    String(&'a [u8]),
    /// A list of items (references to original buffer).
    List(Vec<RlpItem<'a>>),
}
...
impl<'a> RlpItem<'a> {
    ...
    /// Converts string data to a 32-byte value.
    pub fn as_bytes32(&self) -> Option<[u8; 32]> {
        let data = self.as_string()?;
        if data.len() > 32 {
            return None;
        }
        // Right-align for big-endian values
...
///
/// # Arguments
/// * `input` - RLP-encoded data
///
/// # Returns
/// - The decoded item and remaining bytes
...
fn decode_internal(input: &[u8], depth: usize) -> Result<(RlpItem<'_>, &[u8]), RlpError> {
    ...
    match first {
        ...
        0xb8..=0xbf => {
            ...
            // Check that length is >= 56 (otherwise should use short form)
            if len < 56 {
                return Err(RlpError::NonCanonical);
            }
...
/// Encodes a u64 as RLP bytes (for computing unsigned transaction hash).
